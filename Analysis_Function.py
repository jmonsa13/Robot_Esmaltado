...
def analisis_variable(df, idx_variable, variable, flag_var):
    # Iniciación de variables
    tiempo_var = 0
    total_var = 0
    max_var = 0

    while df.iloc[idx_variable][variable] > 0 or df.iloc[idx_variable + 1][variable] > 0:
        ...
        if idx_variable >= df.shape[0] - 1:
            if df.iloc[idx_variable][variable] > 0:
                # Acumulo la data del proceso
                tiempo_var += 1
                total_var += df.iloc[idx_variable][variable]
...
@st.cache(persist=False, allow_output_mutation=True, suppress_st_warning=True, show_spinner=True)
def analitica_esmalte_orig(df, table, periodo):
    """
        ...
        if df.iloc[idx]['estado'] == 1:
            ...
            while df.iloc[idx]['estado'] == 1:
                ...
                if df.iloc[idx]['fmasico'] > 0 and flag_fmasico is True:
                    ...
                    flag_fmasico = analisis_variable(df, idx, "fmasico", flag_fmasico)
                ...
                if df.iloc[idx]['sp_fmasico'] > 0 and flag_sp_fmasico is True:
                    tiempo_sp_fmasico, sp_flujo_masico_total, \
                    max_sp_fmasico, flag_sp_fmasico = analisis_variable(df, idx, "sp_fmasico", flag_sp_fmasico)
                ...
                if df.iloc[idx]['patomizacion'] > 0 and flag_patomizacion is True:
                    tiempo_patomizacion, total_patomizacion, max_patomizacion, \
                    flag_patomizacion = analisis_variable(df, idx, "patomizacion", flag_patomizacion)
                # --------------------------------------------------------------------------------------------
                ...
                if df.iloc[idx]['sp_patomizacion'] > 0 and flag_sp_patomizacion is True:
                    ...
                    flag_sp_patomizacion = analisis_variable(df, idx, "sp_patomizacion", flag_sp_patomizacion)
                # --------------------------------------------------------------------------------------------
                # Conteo del Presion abanico
                if df.iloc[idx]['pabanico'] > 0 and flag_pabanico is True:
                    tiempo_pabanico, total_pabanico, max_pabanico, \
                    flag_pabanico = analisis_variable(df, idx, "pabanico", flag_pabanico)
                # --------------------------------------------------------------------------------------------
                ...
                if df.iloc[idx]['sp_pabanico'] > 0 and flag_sp_pabanico is True:
                    ...
                    flag_sp_pabanico = analisis_variable(df, idx, "sp_pabanico", flag_sp_pabanico)
                # --------------------------------------------------------------------------------------------
                # Avanzo en el df
                idx += 1
...
def analisis_variable_test(df, idx_variable, variable, flag_var, tiempo_var, total_var, max_var):
        ...
        if df.iloc[idx]['estado'] == 1:
            ...
            while df.iloc[idx]['estado'] == 1:
                ...
                while total_flag is True:
                    # --------------------------------------------------------------------------------------------
                    # Conteo del flujo masico
                    tiempo_fmasico, flujo_masico_total, max_flujo_masico, flag_fmasico = analisis_variable_test(df,
                                                                                                                idx_var,
                                                                                                                "fmasico",
                                                                                                                flag_fmasico,
                                                                                                                tiempo_fmasico,
                    ...
                    # --------------------------------------------------------------------------------------------
                    # Conteo del Set point flujo masico
                    tiempo_sp_fmasico, sp_flujo_masico_total, max_sp_fmasico, flag_sp_fmasico = analisis_variable_test(
                        df, idx_var,
                        "sp_fmasico", flag_sp_fmasico, tiempo_sp_fmasico, sp_flujo_masico_total, max_sp_fmasico)
                    # --------------------------------------------------------------------------------------------
                    # Conteo del Presion Atomizacion
                    tiempo_patomizacion, total_patomizacion, max_patomizacion, flag_patomizacion = analisis_variable_test(
                        df, idx_var,
                        "patomizacion", flag_patomizacion, tiempo_patomizacion, total_patomizacion, max_patomizacion)
                    # --------------------------------------------------------------------------------------------
                    # Conteo del  set_point -Presion Atomizacion
                    tiempo_sp_patomizacion, total_sp_patomizacion, max_sp_patomizacion, flag_sp_patomizacion = analisis_variable_test(
                        df, idx_var,
                        "sp_patomizacion", flag_sp_patomizacion, tiempo_sp_patomizacion, total_sp_patomizacion,
                        max_sp_patomizacion)
                    # --------------------------------------------------------------------------------------------
                    # Conteo del Presion abanico
                    tiempo_pabanico, total_pabanico, max_pabanico, flag_pabanico = analisis_variable_test(df, idx_var,
                                                                                                          "pabanico",
                                                                                                          flag_pabanico,
                                                                                                          tiempo_pabanico,
                                                                                                          total_pabanico,
                    ...
                    # --------------------------------------------------------------------------------------------
                    # Conteo del  set_point -Presion abanico
                    tiempo_sp_pabanico, total_sp_pabanico, max_sp_pabanico, flag_sp_pabanico = analisis_variable_test(
                        df, idx_var,
                    ...
                    if flag_fmasico is False and flag_sp_fmasico is False and flag_patomizacion is False and flag_sp_patomizacion is False and flag_pabanico is False and flag_sp_pabanico is False:
                        total_flag = False
